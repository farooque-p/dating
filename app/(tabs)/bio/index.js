...
const index = () => {
  ...
  const updateUserDescription = async () => {
    try {
      const response = await axios.put(
        `${SERVER_URL}/api/v1/user/${userId}/description`,
        {
      ...
      if (response.status === 200) {
        Alert.alert("Success", "User description is updated successfully!");
      }
    } catch (error) {
      console.log("Error while updating user description. ", error);
    }
  };
  ...
  const handleAddImage = async () => {
    try {
      const response = await axios.post(
        `${SERVER_URL}/api/v1/user/${userId}/profile-images`,
        {
          imageUrl: imageUrl,
        }
      );
      ...
      console.log("Error while adding profile image: ", error);
    }
  };
  ...
  const removeTurnOn = async (turnOn) => {
    try {
      const response = await axios.put(
        `${SERVER_URL}/api/v1/user/${userId}/turn-ons/remove`,
        {
          turnOn: turnOn,
        }
      );
  ...
  const addTurnOn = async (turnOn) => {
    try {
      const response = await axios.put(
        `${SERVER_URL}/api/v1/user/${userId}/turn-ons/add`,
        {
          turnOn: turnOn,
        }
      );
  ...
  const addLookingFor = async (lookingFor) => {
    try {
      const response = await axios.put(
        `${SERVER_URL}/api/v1/user/${userId}/looking-for/add`,
        { lookingFor: lookingFor }
      );
      //console.log("Looking For:", response.data);

      if (response.status == 200) {
        setLookingOptions([...lookingOptions, lookingFor]);
      }
    } catch (error) {
      console.log("Error while adding Looking For.");
    }
  };
  ...
  const removeLookingFor = async (lookingFor) => {
    try {
      const response = await axios.put(
        `${SERVER_URL}/api/v1/user/${userId}/looking-for/remove`,
        { lookingFor: lookingFor }
      );
      //console.log("Looking For: ", response.data);
      ...
      console.log("Error while removing Looking For.");
    }
  };
